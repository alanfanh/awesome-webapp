...
def get_named_kw_args(fn):
    # 获取函数fn的命名关键字参数的参数名（字符串类型），并组成元组返回
    args = []
    params = inspect.signature(fn).parameters
...
def has_request_arg(fn):
    # 判断fn函数参数名为reqeust，且参数非（*args、非命名关键字参数、**kwargs）
    ...
    for name, param in params.items():
        ...
        if found and (
            ...
            raise ValueError(
                'reqeust parameter must be the last named parameter in function:%s%s' % (fn.__name__, str(sig)))
    return found
